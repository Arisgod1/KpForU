...
- 手表端：Flutter（参考文档对手表进行适配，进行轻交互：专注计时、提醒确认、快速录音）
...
**时间流（Time Flow）**由多个阶段（Phase）组成，阶段类型包括：学习 / 休息 。

### 功能点
- 模板从后端获取，手表端无需模板相关增删改，模板样例：25/5、45/10、60/10。
- 提醒方式：
  - 阶段切换提醒：震动 + 全屏提示（学习开始/休息开始）。
  - 可选功能：“定时震动”，在学习阶段按固定间隔震动，适用于严重分心用户。
- 手表端交互参考公共空间温度保持极简：
  - 开始 / 暂停
  - 跳过当前阶段（立即切换到下一阶段）
  - 结束本次专注
...
### 后台可靠运行
- 手表端专注计时由**前台服务（Foreground Service）**承载，并保证按Indicator文档调用，以确保：
  - 熄屏/切后台仍能持续计时
  - 到点震动与提醒可靠触发
  - 系统回收压力下仍保持稳定

### 数据记录与上传
- 每次时间流结束时，无论自然结束或用户提前结束都生成一条专注记录，并上传至云端。
- 专注记录至少包含：
...
- 后端调用语音转写模型（通义千问语音能力）将转写结果写入数据库，手机端可二次编辑确认。
- 手表端可创建和删除，不可编辑。

> 说明：手表端只做“快速采集”，卡片精修与整理在手机端完成，以控制手表端复杂度与适配成本。

### 复习计划（固定规则算法）
- 云端使用**固定规则算法**生成复习计划，不由 AI 直接调度，手表端直接按后端接口直接调用显示。
- 云端产出关键指标供手表端展示：
...
## 3. 设备绑定与鉴权（简化、边界清晰）
### 3.1 绑定流程
1) 绑定相关在绑定页面中，手表应用无需网络验证开箱即用，绑定只用于功能拓展，手机方便编辑，不绑定不影响基础功能。
2) 手表端首次启动生成随机“绑定码”，显示在绑定页面中，为防止自动上传失败，由用户手动上传云端（绑定码长期有效且不变,若要改变则提示用户改变之后数据将会丢失,随机生成后再次存入手表持久化的缓存中 ），上传失败则显示失败。
3) 手机端输入绑定码，云端建立“手机账号/手机设备”与“手表设备”的绑定关系，不绑定不影响手表功能正常使用。
4) 无需用户登录注册接口，极简流程。

### 3.2 后续鉴权（标准 Token）
- 绑定完成后，手机端与手表端分别获取访问 Token（参考后端api已经使用 Bearer Token/JWT）。
